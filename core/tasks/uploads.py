...
Design summary (see accompanying explanation for full rationale):
    ...
    Storage (Supabase) → temp local file → openpyxl read-only iteration
        ...
        → allocate_user_and_order_ids / allocate_product_ids  (SQL, set-based)
        → flush_customers_staging / flush_products_staging     (existing, unchanged)
...
def _attach_uploaded_file(upload_record, field_name: str, local_path: str, original_filename: str):
    """
...
def process_customers_upload(self, job_id: str):
    ...
    if not is_valid:
        _mark_failed(job, "mapping_error", f"خطا در نگاشت ستون‌های فایل مشتریان: {error_msg}")
        return
    ...
    try:
        ...
        if total_saved == 0:
            _mark_failed(job, "file_error", "هیچ رکورد معتبری در فایل مشتریان یافت نشد.")
            return

        with connection.cursor() as cursor:
            cursor.execute("SET LOCAL statement_timeout = '1800000';")
            cursor.execute("SELECT allocate_user_and_order_ids(%s)", [tenant.id])
        ...
        job.message = f"{rows_saved} رکورد مشتریان با موفقیت در دیتابیس ذخیره شد."
        job.save(update_fields=["rows_saved", "status", "message", "updated_at"])

    except Exception as e:
        _mark_failed(job, "pipeline_error", f"خطای غیرمنتظره در پردازش فایل مشتریان: {str(e)}")
    finally:
        delete_from_storage(job.storage_key)


def _copy_customers_chunk(tenant_id, chunk_df, mapping, actual_column_mapping, upload_created_at):
    """
    ...
    df["internal_product_id"] = chunk_df.iloc[:, mapping["internal_product_id"]].astype(str)
    df["first_name"] = chunk_df.iloc[:, mapping["first_name"]].astype(str).str.slice(0, 200)
    df["last_name"] = chunk_df.iloc[:, mapping["last_name"]].astype(str).replace("", None)
    df["gender"] = chunk_df.iloc[:, mapping["gender"]].astype(str).replace("", None)
    df["phone_number"] = (
        chunk_df.iloc[:, mapping["phone_number"]].astype(str).str.slice(0, 20).replace("", None)
    )
    ...
    df["quantity"] = pd.to_numeric(
        chunk_df.iloc[:, mapping["quantity"]], errors="coerce"
    ).fillna(0).astype(int)
    ...
    valid_mask = (
        (df["internal_user_id"] != "") & (df["internal_user_id"] != "None")
        & (df["internal_order_id"] != "") & (df["internal_order_id"] != "None")
        & (df["internal_product_id"] != "") & (df["internal_product_id"] != "None")
    )
    df = df[valid_mask]
    ...
    _copy_buffer_into(
        ...
        COPY users_unnormalized_data_staging (
            tenant_id, internal_user_id, first_name, last_name,
...
def process_products_upload(self, job_id: str):
    ...
    if not is_valid:
        _mark_failed(job, "mapping_error", f"خطا در نگاشت ستون‌های فایل محصولات: {error_msg}")
        return
    ...
    try:
        ...
        if total_saved == 0:
            _mark_failed(job, "file_error", "هیچ رکورد معتبری در فایل محصولات یافت نشد.")
            return

        with connection.cursor() as cursor:
            cursor.execute("SET LOCAL statement_timeout = '1800000';")
            cursor.execute("SELECT allocate_product_ids(%s)", [tenant.id])
        ...
        job.message = f"{rows_saved} رکورد محصولات با موفقیت در دیتابیس ذخیره شد."
        job.save(update_fields=["rows_saved", "status", "message", "updated_at"])

    except Exception as e:
        _mark_failed(job, "pipeline_error", f"خطای غیرمنتظره در پردازش فایل محصولات: {str(e)}")
    finally:
        delete_from_storage(job.storage_key)


def _copy_products_chunk(tenant_id, chunk_df, mapping, actual_column_mapping, upload_created_at):
    df = pd.DataFrame()
    df["internal_product_id"] = chunk_df.iloc[:, mapping["internal_product_id"]].astype(str)
    df["product_name"] = chunk_df.iloc[:, mapping["product_name"]].astype(str).str.slice(0, 255)
    ...
    df["first_product_attribute"] = (
        first_attr_raw.astype(str).str.strip().replace({"": None, "nan": None, "None": None})
    )
    df["second_product_attribute"] = (
        second_attr_raw.astype(str).str.strip().replace({"": None, "nan": None, "None": None})
    )

    rows_processed = len(df)

    valid_mask = (df["internal_product_id"] != "") & (df["internal_product_id"] != "None")
    df = df[valid_mask]
    ...
    _copy_buffer_into(
        ...
        COPY products_unnormalized_data_staging (
            tenant_id, internal_product_id, product_name, product_category,
...
def process_coupons_upload(self, job_id: str):
    ...
    if not is_valid:
        _mark_failed(job, "mapping_error", f"خطا در نگاشت ستون‌های فایل کوپن: {error_msg}")
        return
    ...
    try:
        ...
        _mark_failed(job, "pipeline_error", f"خطای غیرمنتظره در ذخیره‌سازی کوپن‌ها: {str(e)}")
    finally:
        delete_from_storage(job.storage_key)
