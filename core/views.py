...
class CampaignCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        campaign.tenant = Tenant.objects.get(owner=self.request.user)
        campaign.save()
        messages.success(self.request, "کمپین با موفقیت ایجاد شد.")
...
class CampaignUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_object(self):
        return get_object_or_404(
            Campaign, pk=self.kwargs["pk"], tenant__owner=self.request.user
        )
    ...
    def form_valid(self, form):
        ...
        campaign.tenant = Tenant.objects.get(owner=self.request.user)
        campaign.save()
        messages.success(self.request, "کمپین با موفقیت به‌روزرسانی شد.")
