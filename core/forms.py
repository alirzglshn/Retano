...
from .utils.excel_mapper import CustomerExcelMapper, ProductExcelMapper, CouponExcelMapper
...
class CampaignForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "comparison_value": forms.NumberInput(
                attrs={"class": "form-control", "placeholder": "مقدار عددی وارد کنید",
                       "step": "1", "min": "0"}
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance and self.instance.pk and hasattr(self.instance, "campaign_date"):
...
class ColumnMappingForm(forms.Form):
    ...
    def clean(self):
        ...
        if has_customers:
            ...
            idxs = [cleaned_data.get(f) for f in self._CUSTOMERS_FORM_FIELDS
        ...
        if has_products:
            ...
            idxs = [cleaned_data.get(f) for f in self._PRODUCTS_FORM_FIELDS
                    if cleaned_data.get(f) is not None]
            if len(idxs) != len(set(idxs)):
                raise forms.ValidationError(
        ...
        if has_coupons:
            ...
            idxs = [cleaned_data.get(f) for f in self._COUPONS_FORM_FIELDS
                    if cleaned_data.get(f) is not None]
            if len(idxs) != len(set(idxs)):
                raise forms.ValidationError(
